...
if renew_files:
    ...
    vote_metadata.to_pickle(DATA_PATH + "vote_metadata.pkl")
    roll_call_votes.to_pickle(DATA_PATH + "roll_call_votes.pkl")
    leg_data.to_pickle(DATA_PATH + "leg_data.pkl")
    party_data.to_pickle(DATA_PATH + "party_data.pkl")
else:
    vote_metadata = pd.read_pickle(DATA_PATH + "vote_metadata.pkl")
    roll_call_votes = pd.read_pickle(DATA_PATH + "roll_call_votes.pkl")
    leg_data = pd.read_pickle(DATA_PATH + "leg_data.pkl")
    party_data = pd.read_pickle(DATA_PATH + "party_data.pkl")

leg_party = leg_data[["leg_id", "party_code"]].drop_duplicates()
leg_party[leg_party["leg_id"].duplicated(keep=False)].sort_values("leg_id")

# Map the vote categories
yea_codes = [1, 2, 3]
nay_codes = [4, 5, 6]
present_codes = [7, 8]
...
# Subset to valid votes
vote_df = roll_call_votes.dropna(subset=["vote"]).copy()
...
while voter_condition or unanimity_condition:
    ...
    valid_legs = leg_vote_counts[leg_vote_counts > min_vote_count].index.tolist()
    vote_df = vote_df[vote_df["leg_id"].isin(valid_legs)]
    ...
    nonunanimous_vote_ids = nonunanimous_votes.index.tolist()
    ...
    vote_df = vote_df[vote_df["vote_id"].isin(nonunanimous_vote_ids)]
...
vote_df.to_pickle(DATA_PATH + "raw_vote_df.pkl")
...
vote_df_temp.to_pickle(DATA_PATH + "vote_df.pkl")
