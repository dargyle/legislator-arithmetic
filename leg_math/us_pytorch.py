...
train_loss = loss(preds.mean(axis=0), responses).item()
logger.info("Train loss: {train_loss}")
train_accuracy = accuracy_score(responses, 1 * (preds.mean(axis=0) >= 0.5))
...
test_accuracy = accuracy_score(responses_test, 1 * (preds.mean(axis=0) >= 0.5))
